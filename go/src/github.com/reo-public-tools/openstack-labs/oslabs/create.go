...
func Create(configFile string) (error) {
    ...
    // Parse the config for an mnaio only setup.
    var mnaioonly bool = true
    for _, foremanhost := range labConfig.Environment.ForemanHosts {
        if foremanhost.Role != "simulator" {
            mnaioonly = false
        }
        if foremanhost.Role == "simulator" && foremanhost.SimulatorType != "mnaio" {
            mnaioonly = false
        }
    }
    ...
    for _, foremanHost := range labConfig.Environment.ForemanHosts {
        ...
        // Skip mnaio simulator
        if foremanHost.Role == "simulator" && foremanHost.SimulatorType == "mnaio" {
            fmt.Printf("Skipping mnaio simulator node\n")
            continue
        }
    ...
    /* #### Start simulated host buildout #### */

    // Set up the channel for communications
    simulatedHostInfoChan := make(chan hostCreateRet)

    // Track the number of working goroutines
    var swg sync.WaitGroup

    // Loop through the foreman hosts in the lab config and create
    for _, simulatedHost := range labConfig.Environment.SimulatedHosts {

        // Name the host based on role and increment it.
        for i := 1; i <= simulatedHost.Count; i++ {

            // Set up the hostname
            hostName := fmt.Sprintf("%s%0.2d", simulatedHost.Role, i)
            ...
            go func(url string, session string, labName string, hostName string, hostGroup string, role string) {
                ...
                // Create the host and send the results back to the main goroutine
                fmt.Printf("Creating new simulated node %s.%s in host group %s\n", hostName, labName, hostGroup)
                hostRet.hostInfo, hostRet.err = theforeman.CreateSimulatedHost(url, session, labName, hostName, hostGroup, role)
                simulatedHostInfoChan <- hostRet

            } (labConfig.ForemanURL, session, curdomaininfo.Name, hostName, simulatedHost.ForemanHostgroup, simulatedHost.Role)
    ...
    go func() {
        swg.Wait()
        close(simulatedHostInfoChan)
    }()

    // Loop over results
    for hostRetInfo := range simulatedHostInfoChan {
        if hostRetInfo.err != nil {
            fmt.Println(hostRetInfo.err)
        } else {
            fmt.Printf("Finished creating host %s with primay ip of %s.\n", hostRetInfo.hostInfo.Name, hostRetInfo.hostInfo.IP)
